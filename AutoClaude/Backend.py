...
async def save_join_config(guild_id: str, data: Dict = Body(...)):
    db_manager.save_config(guild_id, "join", data or {})
    return {"success": True, "config": db_manager.get_config(guild_id, "join")}
...
async def save_dm_config(guild_id: str, data: Dict = Body(...)):
    db_manager.save_config(guild_id, "dm", data or {})
    return {"success": True, "config": db_manager.get_config(guild_id, "dm")}
...
async def main():
    ...
    # 1. Initialize the single Master Bot
    bot = IntegratedCommandBot(db_manager)
    global command_bot_instance
    command_bot_instance = bot
    
    # 2. Initialize Feature Components
    leave_server = LeaveServer(db_manager)
    join_server = JoinServer(db_manager)
    pm_server = PrivateMessageServer(db_manager)
    jr_server = JoinRoleServer(db_manager)
    lvl_server = LevelServer(db_manager, msg_db)
    rr_bot = ReactionRoles(db_manager)
    sa_sender = ScheduledAnnouncementSender(db_manager)
    hi_reactor = HiEmojiReactor(db_manager)
    
    # Message Logger
    msg_logger = MessageLoggerClient(db_manager, msg_db)
    global msg_logger_instance
    msg_logger_instance = msg_logger
    
    global rr_bot_instance
    rr_bot_instance = rr_bot

    # 3. Patch components to use the master bot's connection and state
    # This ensures self.get_guild, self.user, etc. work inside component classes
    def patch_component(obj, master_bot):
        # Store master bot reference
        obj.master_bot = master_bot
        
        # Proxy common discord.Client methods to the master bot
        obj.get_guild = master_bot.get_guild
        obj.get_channel = master_bot.get_channel
        obj.fetch_channel = master_bot.fetch_channel
        obj.get_user = master_bot.get_user
        obj.fetch_user = master_bot.fetch_user
        
        # Redirect the 'user' attribute access to master_bot.user
        # We use a custom __class__ hack or just rely on the fact that these are listeners
        # For IntegratedCommandBot components, we'll just ensure they have what they need
        if hasattr(obj, 'start_bot'):
            # Disable their own start_bot to prevent accidental separate connections
            obj.start_bot = lambda: asyncio.sleep(0) 

    components = [leave_server, join_server, pm_server, jr_server, lvl_server, rr_bot, sa_sender, hi_reactor, msg_logger]
    
    for comp in components:
        patch_component(comp, bot)
        
        # 4. Register Event Listeners to the Master Bot
        if hasattr(comp, 'on_member_remove'): bot.add_listener(comp.on_member_remove)
        if hasattr(comp, 'on_member_join'): bot.add_listener(comp.on_member_join)
        if hasattr(comp, 'on_message'): bot.add_listener(comp.on_message)
        if hasattr(comp, 'on_raw_reaction_add'): bot.add_listener(comp.on_raw_reaction_add)
        if hasattr(comp, 'on_raw_reaction_remove'): bot.add_listener(comp.on_raw_reaction_remove)
        if hasattr(comp, 'on_message_edit'): bot.add_listener(comp.on_message_edit)
        if hasattr(comp, 'on_message_delete'): bot.add_listener(comp.on_message_delete)
        
        # Special case for ScheduledAnnouncements poll loop
        if isinstance(comp, ScheduledAnnouncementSender):
            bot.add_listener(comp.on_ready)
