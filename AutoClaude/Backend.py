...
from Core.Health_Metrics import HealthMetricsService
from Core.Leaderboard_Pro import LeaderboardProService
from Core.Moderation_Pro import ModerationProService
from Core.Network_Pro import NetworkProService
from Core.Profile_Pro import ProfileProService
from Core.Achievements_Pro import AchievementsService
from Core.Lifecycle_Pro import LifecycleProManager
from Message_Database import MessageDBHandler, MessageLoggerClient

msg_db = MessageDBHandler()
health_pro_service = HealthMetricsService(msg_db)
leaderboard_pro_service = LeaderboardProService(msg_db)
moderation_pro_service = ModerationProService(msg_db, db_manager)
network_pro_service = NetworkProService(msg_db)
profile_pro_service = ProfileProService(msg_db, db_manager)
achievements_pro_service = AchievementsService(msg_db)
...
@app.get("/gui/Commands.html")
async def get_commands_html():
    path = os.path.join(BASE_DIR, "GUI", "Commands.html")
    if os.path.exists(path):
        with open(path, "r", encoding="utf-8") as f:
            return HTMLResponse(content=f.read(), headers={"Cache-Control": "no-cache, no-store, must-revalidate"})
    return JSONResponse({"error": "Commands.html not found"}, status_code=404)

@app.get("/gui/Setting.html")
async def get_setting_html():
    path = os.path.join(BASE_DIR, "GUI", "Setting.html")
    if os.path.exists(path):
        with open(path, "r", encoding="utf-8") as f:
            return HTMLResponse(content=f.read(), headers={"Cache-Control": "no-cache, no-store, must-revalidate"})
    return JSONResponse({"error": "Setting.html not found"}, status_code=404)

@app.get("/gui/Level.html")
async def get_level_html():
    path = os.path.join(BASE_DIR, "GUI", "Level.html")
    if os.path.exists(path):
        with open(path, "r", encoding="utf-8") as f:
            return HTMLResponse(content=f.read(), headers={"Cache-Control": "no-cache, no-store, must-revalidate"})
    return JSONResponse({"error": "Level.html not found"}, status_code=404)

@app.get("/gui/Leaderboard.html")
async def get_leaderboard_html():
    path = os.path.join(BASE_DIR, "GUI", "Leaderboard.html")
    if os.path.exists(path):
        with open(path, "r", encoding="utf-8") as f:
            return HTMLResponse(content=f.read(), headers={"Cache-Control": "no-cache, no-store, must-revalidate"})
    return JSONResponse({"error": "Leaderboard.html not found"}, status_code=404)
...
@app.get("/api/settings/commands-{cmd}")
async def get_generic_command_settings(cmd: str):
    key = f"commands_{cmd.replace('-', '_')}"
    settings = db_manager.get_settings().get(key, {})
    if not isinstance(settings, dict):
        settings = {"enabled": False, "role_ids": []}
    return {
        "success": True,
...
@app.post("/api/settings/commands-{cmd}")
async def save_generic_command_settings(cmd: str, payload: Dict = Body(...)):
    key = f"commands_{cmd.replace('-', '_')}"
    enabled = bool((payload or {}).get("enabled", False))
    role_ids = (payload or {}).get("role_ids", [])
    if not isinstance(role_ids, list):
        role_ids = []
    db_manager.save_settings({key: {"enabled": enabled, "role_ids": role_ids}})
...
@app.get("/api/pro/health/{guild_id}")
async def get_pro_health(guild_id: str, hours: int = 24):
    safe_hours = max(1, min(int(hours), 168))
    payload = health_pro_service.get_health_overview(guild_id, hours=safe_hours)
    return {"success": True, **payload}


@app.get("/api/pro/leaderboard/{guild_id}")
async def get_pro_leaderboard(guild_id: str, period: str = "all", metric: str = "xp", limit: int = 25, q: str = ""):
    safe_limit = max(1, min(int(limit), 100))
    rows = leaderboard_pro_service.get_rankings(
        guild_id=guild_id,
        period=period,
        metric=metric,
        limit=safe_limit,
    ...
    return {"success": True, "guild_id": str(guild_id), "period": period, "metric": metric, "rows": rows}


@app.get("/api/pro/moderation/{guild_id}/rules")
async def get_pro_moderation_rules(guild_id: str):
    rules = moderation_pro_service.get_rules(guild_id)
    actions = moderation_pro_service.get_recent_actions(guild_id, limit=20)
    return {"success": True, "rules": rules, "actions": actions}


@app.post("/api/pro/moderation/{guild_id}/rules")
async def save_pro_moderation_rule(guild_id: str, payload: Dict = Body(...)):
    rule = moderation_pro_service.add_rule(
        guild_id=guild_id,
        rule_type=(payload or {}).get("type", "blocklist"),
        pattern=(payload or {}).get("pattern", ""),
        action=(payload or {}).get("action", "warn"),
        reason=(payload or {}).get("reason", "Pro moderation rule"),
        creator_id=(payload or {}).get("creator_id", "api")
    )
    if not rule:
...
@app.delete("/api/pro/moderation/{guild_id}/rules/{rule_id}")
async def delete_pro_moderation_rule(guild_id: str, rule_id: int):
    moderation_pro_service.remove_rule(guild_id, rule_id)
    return {"success": True}


@app.post("/api/pro/moderation/{guild_id}/triage")
async def triage_pro_moderation(guild_id: str, payload: Dict = Body(...)):
    result = moderation_pro_service.triage_message(
        guild_id=guild_id,
        content=(payload or {}).get("content", ""),
        user_id=(payload or {}).get("user_id"),
        message_id=(payload or {}).get("message_id"),
        actor_id=(payload or {}).get("actor_id", "api")
    )
    return {"success": True, "result": result}


@app.get("/api/pro/network/{guild_id}")
async def get_pro_network(guild_id: str, hours: int = 72, limit: int = 80):
    safe_hours = max(1, min(int(hours), 720))
    safe_limit = max(5, min(int(limit), 250))
    graph = network_pro_service.build_graph(guild_id=guild_id, hours=safe_hours, limit=safe_limit)
    return {"success": True, **graph}


@app.get("/api/pro/profile/{guild_id}/search")
async def search_pro_profiles(guild_id: str, q: str = "", limit: int = 20):
    safe_limit = max(1, min(int(limit), 100))
    rows = profile_pro_service.search_profiles(guild_id, query=q, limit=safe_limit)
    return {"success": True, "rows": rows}


@app.get("/api/pro/profile/{guild_id}/{user_id}")
async def get_pro_profile(guild_id: str, user_id: str):
    profile = await profile_pro_service.get_profile(guild_id, user_id)
...
@app.get("/api/achievements/stats/{guild_id}")
async def get_achievement_stats(guild_id: str):
    stats = achievements_pro_service.get_stats(guild_id)
    return {"success": True, "achievements": stats}

@app.get("/api/achievements/members/{guild_id}/{achievement_id}")
async def get_achievement_members(guild_id: str, achievement_id: str):
    members = achievements_pro_service.get_earners(guild_id, achievement_id)
...
async def main():
    ...
    global command_bot_instance
    command_bot_instance = IntegratedCommandBot(db_manager, moderation_pro_service=moderation_pro_service)
    ...
    global lifecycle_pro_instance
    lifecycle_pro_instance = LifecycleProManager(db_manager, msg_db, command_bot_instance)
    await lifecycle_pro_instance.start()
    ...
    try:
        ...
        if lifecycle_pro_instance:
            await lifecycle_pro_instance.stop()
