...
from Core.Command.Leaderboard_Command import LeaderboardQueryView, build_ui_text
...
class IntegratedCommandBot(commands.Bot):
    def __init__(self, db, moderation_pro_service=None):
        ...
        self.db = db
        self.token = db.token
        self.moderation_pro_service = moderation_pro_service
    ...
    async def setup_hook(self):
        ...
        async def leaderboard(interaction: discord.Interaction):
            ...
            try:
                ...
                return
        # 5. Load New Command Cogs
        cogs = [
            "Achievements", "Afk", "Autorole", "Backup", "Event", "Events",
            "Giveaway_Export", "Health_Check", "Help", "Help_Admin",
            "Leaderboard_Post", "Level_Rewards", "Moderation", "Ping",
            "Poll", "Profile", "Punishment", "Quests", "Reputation_Config",
            "Role", "Role_Config", "Thank", "Ticket", "Ticket_Setup", "Xp"
        ]
        
        for cog in cogs:
            try:
                await self.load_extension(f"Core.Command.{cog}")
    ...
    def _check_perms(self, interaction, cfg):
        ...
        Returns True if the user can use the command, False otherwise.
        When False, caller must NOT send any response (this function already scheduled an error).
        """
        # Administrators always bypass role checks
        ...
        # If no role restriction is set, allow everyone
        if not allowed:
            return True
