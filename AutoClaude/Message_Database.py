    ...
    def rebuild_user_levels_from_messages(self, guild_id, xp_min=15, xp_max=25, formula=None):
        ...
        cursor.execute(
            ...
            SELECT user_id, username, user_discriminator, avatar_url,
                   content_length, word_count, created_at, bot_message, is_deleted, xp_gain
            FROM messages
            WHERE guild_id=?
            """,
            (str(guild_id),)
        ...
        for row in rows:
            ...
            # Generate random XP for historical messages if not already set
            msg_xp = int(row["xp_gain"] or 0)
            if msg_xp == 0:
                msg_xp = random.randint(int(xp_min), int(xp_max))
                # Update the message record so health metrics can see it
                cursor.execute("UPDATE messages SET xp_gain=? WHERE message_id=?", (msg_xp, str(row["message_id"])))
            
            u["xp_total"] += msg_xp
            
            last_ts = float(row["created_at"] or 0)
            if last_ts > u["last_message_at"]:
                u["last_message_at"] = last_ts

        now = time.time()
        for uid, u in users.items():
            level, xp_current = self._compute_level_from_total(u["xp_total"], formula)
            u["level"] = level
            u["xp_current"] = xp_current
            u["updated_at"] = now
...
class MessageLoggerClient(BaseDiscordClient):
    ...
    async def download_history_background(self, guild_id, status_tracker):
        ...
        # Combine text channels and active threads
        ...
        for channel in all_channels:
            try:
                ...
                if not getattr(permissions, 'read_message_history', False) or not getattr(permissions, 'read_messages', False):
                    print(f"[MessageLogger] Skipping {channel.name}: Missing permissions")
                    continue
                ...
                except (discord.Forbidden, discord.HTTPException) as eh:
                    print(f"[MessageLogger] Error reading history from {channel.name}: {eh}")
                    continue
                ...
                print(f"[MessageLogger] Unexpected error in {channel.name}: {e}")
        
        status_tracker["status"] = "rebuilding"
        status_tracker["progress_text"] = "Updating user XP totals from history..."
        ...
        try:
            ...
            print(f"[MessageLogger] Rebuild error: {e}")
            status_tracker["status"] = "error"
