...
window.initializeJoinServer = function() {
    ...
    function applyJoinCardUIFromState() {
        ...
        const titlePreview = document.getElementById('join_card_title_text');
    ...
    window.updateCardValue = function(feature, key, val) {
        ...
        if (key === 'titleSize' || key === 'subtitleSize') {
            applyJoinCardFontSizes();
        }
        checkForChanges();
    };
    ...
    function applyJoinCardFontSizes() {
        const card = state.join.card || defaultCardState;
        const titleEl = document.getElementById('join_card_title_text');
        const subtitleEl = document.getElementById('join_card_subtitle_text');
        if (titleEl && card.titleSize) titleEl.style.fontSize = `${card.titleSize}px`;
        if (subtitleEl && card.subtitleSize) subtitleEl.style.fontSize = `${card.subtitleSize}px`;
        fitJoinCardText(titleEl, card.titleSize || 56, 24);
        fitJoinCardText(subtitleEl, card.subtitleSize || 36, 18);
    }

    function fitJoinCardText(el, baseSize, minSize) {
        if (!el) return;
        const container = el.parentElement;
        if (!container) return;
        let size = parseInt(baseSize, 10) || 0;
        const maxWidth = container.clientWidth ? (container.clientWidth - 20) : 360;
        ...
        joinDataLoadPromise = (async () => {
            ...
            try {
                ...
                if (confData.success && confData.config) {
                    ...
                    if (state.join.mode === 'card' || state.join.card?.enabled) {
                        const cardToggle = document.getElementById('join_card_toggle');
                        if (cardToggle) cardToggle.checked = true;
                        toggleCard('join', true);
                    }
                    if (state.join.type) {
                        setJoinMessageType(state.join.type, true);
                    }
                    checkForChanges();
                }
    ...
    window.addEventListener('resize', applyJoinCardFontSizes);
};
