        ...
        <div class="unified-preview">
            <div class="welcome-card-preview w-full max-w-[520px] mx-auto" id="join_card_preview_wrap">
                ...
                <div class="card-content">
                    ...
                    <div class="card-title" id="join_card_title_text">User#0000 just joined the server</div>
...
window.initializeJoinServer = function() {
    ...
    function fitJoinCardText(el, baseSize, minSize) {
        if (!el) return;
        const maxWidth = getJoinCardMaxWidth();
    ...
    function applyJoinCardFontSizes() {
        ...
        fitJoinCardText(titleEl, titleSize, 24);
        fitJoinCardText(subtitleEl, subtitleSize, 18);
    }
    ...
    window.toggleCard = function(feature, enabled) {
        ...
        if (enabled) applyJoinCardFontSizes();
        checkForChanges();
    };
    ...
    ensureJoinState();
    if (!window.__joinCardResizeBound) {
        window.addEventListener('resize', () => {
            if (state.join && state.join.card && state.join.card.enabled) {
                applyJoinCardFontSizes();
            }
        });
        window.__joinCardResizeBound = true;
    }
    loadDiscordData();
